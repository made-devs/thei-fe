        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {dictionary.services.map((service) => (
            <Link
              href="#"
              key={service.id}
              className="group bg-white p-8 rounded-lg shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all duration-300 flex flex-col text-left"
            >
              {iconMap[service.icon]}
              <h3 className="font-bold text-xl text-black mt-4">
                {service.title}
              </h3>
              <p className="text-gray-600 text-sm mt-2 flex-grow">
                {service.description}
              </p>
              <div className="mt-6 font-semibold text-yellow-500 flex items-center group-hover:text-black transition-colors">
